  ...
  for(let i =0;i<height;i++){
    let temp = []
      for(let j=0;j<width;j++){
        ...
        // else if(props.grid[i][j]===2 && props.visualise){
        //   temp.push(<div  key={i+j} className="animation-target " style={{width:'30px',height:'30px',backgroundColor:'#8ccbbe',WebkitUserSelect: 'none'}}></div>)
        
        // }
        
        else if(props.grid[i][j]===1){
          temp.push(<div key={i+j} className="animation-target" style={{width:'30px',height:'30px',backgroundColor:'#202060',WebkitUserSelect: 'none'}} onTouchStart={(e)=> {if(window.event.buttons===1&&!props.visualise){props.toggleWall(i,j,1)};}}   onMouseOver={(e)=> {if(window.event.buttons===1&&!props.visualise){props.toggleWall(i,j,1)};}} onClick={(e)=> {if(!props.visualise)props.toggleWall(i,j,1);}} ></div>)
        }
        else{
          temp.push(<div  key={i+j}  style={{width:'30px',height:'30px',border:'1px solid lightblue',WebkitUserSelect: 'none'}} onTouchStart={(e)=> {if(window.event.buttons===1&&!props.visualise){props.toggleWall(i,j,0)}}}  onMouseOver={(e)=> {if(window.event.buttons===1&&!props.visualise){props.toggleWall(i,j,0)}}} onClick={(e)=> {if(!props.visualise)props.toggleWall(i,j,0);}}></div>)
        }
