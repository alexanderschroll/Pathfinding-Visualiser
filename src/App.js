...
import logo from './images/algorithm.svg';
import Grid from "./grid";
import PriorityQueue from './priorityq';
import end from './images/circle.svg';
import start from './images/right.svg';
export class App extends Component {
  ...
  this.setState({visualise:false})
  }
  visualiseDijkstra =  async () => {
...
grid:state.grid.map(row => row.map(obj => {
 ...
 <div className="dropdown mx-4">
  <button className="btn  dropdown-toggle" type="button" id="dropdownMenuButton" style={{backgroundColor:'#dcd6f7',}}  data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" disabled={this.state.visualise}>
    Algorithm
  </button>
 ...
 <button className=" btn mx-4" onClick={()=>this.state.method===0?this.visualiseDijkstra():this.visualiseA()} style={{backgroundColor:'#dcd6f7',}} disabled={this.state.visualise}>
Visualise {this.state.method===0?" Dijkstra's Algorithm " : " A* "}
 </button>
 <div className="dropdown mx-4">
  <button className="btn dropdown-toggle" type="button" id="dropdownMenuButton" style={{backgroundColor:'#dcd6f7',}}  data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" disabled={this.state.visualise}>
 ...
 <button className=" btn mx-4" onClick={() => this.setState({grid:Array(20).fill().map(() => Array(40).fill(0)),path:[]})} style={{backgroundColor:'#dcd6f7',}} disabled={this.state.visualise}>
 Clear Walls
 </button>
 <button className=" btn mx-4" onClick={()=>this.clearPath()} style={{backgroundColor:'#dcd6f7',}} disabled={this.state.visualise}>
 Clear Path
 </button>
 </div>
      </div>
      ...
      <Grid height={this.state.height} grid={this.state.grid} path={this.state.path} current={this.state.current}  start={this.state.start} end={this.state.end} width={this.state.width} visualise={this.state.visualise} toggleWall={this.toggleWall}/>
